...
{    
  //  if(m_flags & effFlagsCanDoubleReplacing)
  //  {
    if(m_inputsdouble)
...
void RemotePluginServer::dispatchParEvents()
{
    ...
    case RemotePluginGetVersion:
        writeInt(&m_shm[FIXED_SHM_SIZE], getVersion());
        break;
...
*/
    ...
    error = 0;
        ...
        update = readIntring(&m_shmControl5->ringBuffer);
        ...
        writeInt(&m_shm[FIXED_SHM_SIZE], 9999);        
        break;
    }
    ...
    error = 0;
        ...
        if(!update)
        numout = getOutputCount();
        else
        {
        if(aeffectio)
        numout = aeffectio->numInputs;
        else
        break;
        }
   ...
   /* 
    ...
    case RemotePluginGetOutputCount:
    {
        ...
        if(numout != m_numOutputs && m_flags)
        {
        if(m_flags & effFlagsCanDoubleReplacing)
        {
            if(m_outputsdouble)
